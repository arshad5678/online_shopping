...
const COLLECTIONS = [
  {
    ...
    description: "Embrace the beauty of simplicity with our earth-toned essentials.",
  ...
  {
    title: "Ethereal Silk",
    description: "Flowing silhouettes and luxurious fabrics for effortless grace.",
      ...
      <div className="container mx-auto px-4">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div className="max-w-xl">
            <h2 className="text-4xl font-serif font-bold mb-4 tracking-tight text-primary">Trending Collections</h2>
            <p className="text-muted-foreground">
              Our latest curations define the contemporary fashion landscape, blending innovation with heritage.
            </p>
          </div>
          <Button variant="link" className="text-primary font-bold uppercase tracking-widest text-[10px] p-0 h-auto group">
            Discover More <span className="ml-2 transition-transform group-hover:translate-x-1">→</span>
          </Button>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {COLLECTIONS.map((item, idx) => (
            <div key={idx} className="group relative aspect-[3/4] overflow-hidden bg-muted">
              <img 
                src={item.image} 
                alt={item.title} 
              ...
              <div className="absolute inset-0 p-8 flex flex-col justify-end">
                ...
                <div className="transform translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 delay-150">
                  <Button variant="outline" className="text-white border-white hover:bg-white hover:text-black rounded-none text-[10px] font-bold uppercase tracking-widest px-6 h-10 transition-colors">
                    Shop Collection
                  </Button>
                </div>
              </div>
